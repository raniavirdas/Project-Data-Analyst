SELECT  week, 
        ...
        SUM(unrecognized_img_reason)*100/SUM(fail) AS unrecognized_img_reason, 
        SUM(invalid_selfie_reason)*100/SUM(fail) AS invalid_selfie_reason,
        SUM(nik_not_found_reason)*100/SUM(fail) AS nik_not_found_reason, 
        SUM(temporary_network_error_reason)*100/SUM(fail) AS temporary_network_error_reason
FROM
    ...
    WHERE   log_response_key IS NOT NULL 
            ...
            [[AND log_response_created_datetime >= {{From}}]]
            [[AND log_response_created_datetime <= {{To}}]]
    )
GROUP BY 1
ORDER BY 1 ASC;