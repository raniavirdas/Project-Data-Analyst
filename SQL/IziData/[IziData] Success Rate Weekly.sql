...
SELECT  week, 
        ...
        COUNT(DISTINCT CASE WHEN user_category = 'success_only' THEN company_id END)*100/COUNT(DISTINCT company_id) AS success_only, 
        COUNT(DISTINCT CASE WHEN user_category = 'failed_only' THEN company_id END)*100/COUNT(DISTINCT company_id) AS failed_only,
        COUNT(DISTINCT CASE WHEN user_category = 'failed_then_success' THEN company_id END)*100/COUNT(DISTINCT company_id) AS failed_then_success
FROM users_category
GROUP BY 1
ORDER BY 1 asc;