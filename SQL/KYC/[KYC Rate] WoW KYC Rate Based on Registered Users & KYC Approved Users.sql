WITH kyc_docs AS (
  SELECT
        ...
        DATE_TRUNC(MIN(kyc_approved_at), week) AS week,
        final_status
  FROM
      (
        SELECT
...
kyc_informations AS (
    SELECT
          ...
          COALESCE(week,DATE_TRUNC(kyc_updated_date,week)) AS week,
...
companies AS (
  SELECT
        ...
        DATE_TRUNC(company_registered_datetime, week) AS company_registered_week,
...
agg_registered_users AS (
    SELECT 
            DATE(company_registered_week) AS week,
            COUNT(DISTINCT company_id) AS total_users
    FROM companies 
    WHERE company_registered_week >= '2023-01-01' 
...
agg_kyc AS (
    SELECT 
        DATE_TRUNC(first_kyc_approved_date, week) AS week,
...
agg_kyc_rate AS (
    ...
    LEFT JOIN agg_kyc ak ON ak.week = au.week
),
...
raw_data AS (
    ...
    LEFT JOIN agg_kyc ak ON aru.week = ak.week
    LEFT JOIN agg_kyc_rate akr ON akr.week = aru.week
    GROUP BY 1
)
